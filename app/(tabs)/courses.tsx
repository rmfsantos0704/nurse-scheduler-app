...
export default function Courses() {
  ...
  const fetchCourseDetail = async (course: Course) => {
    ...
    setDetailModal(true);
    try {
      const res = await fetch(`${API_URL}/courses/${course._id}/schedules`);
      const data = await res.json();
      setCourseSchedules(Array.isArray(data.schedules) ? data.schedules : []);
    } catch { setCourseSchedules([]); }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setEditModal(false);
    } catch { Alert.alert("Error", "Could not update course."); }
    finally { setSaving(false); }
  };

  const handleDelete = (course: Course) => {
    Alert.alert("Delete course", `Remove "${course.name}"? This will not delete linked schedules.`, [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          await fetch(`${API_URL}/courses/${course._id}`, { method: "DELETE" }).catch(() => {});
        },
      },
    ]);
  };
  ...
  const openEdit = (course: Course) => {
    setSelectedCourse(course);
    setForm({ name: course.name, code: course.code, instructor: course.instructor, room: course.room, color: course.color, timeslots: course.timeslots });
    setDetailModal(false);
    ...
    <>
      ...
      >
        <View style={s.headerRow}>
          ...
          <TouchableOpacity style={[s.addBtn, { backgroundColor: colors.primary }]} onPress={() => { setForm(EMPTY_COURSE); setAddModal(true); }}>
        ...
        {courses.length === 0 ? (
          <View style={s.empty}>
            ...
            <Text style={[s.emptySub, { color: colors.textSecondary }]}>Add your subjects to track activities and schedules per course.</Text>
          </View>
        ) : (
          <View style={s.grid}>
            {courses.map(course => (
              <TouchableOpacity
                key={course._id}
                style={[s.courseCard, { backgroundColor: colors.card, borderColor: colors.cardBorder, borderTopColor: course.color, borderTopWidth: 4 }]}
              ...
              >
                ...
                <Text style={[s.courseName, { color: colors.textPrimary }]} numberOfLines={2}>{course.name}</Text>
                {course.instructor ? (
                ...
                {course.timeslots.length > 0 && (
                  <View style={s.timeslotPreview}>
                    ...
                    {course.timeslots.length > 2 && (
                      <Text style={[s.moreTs, { color: colors.textSecondary }]}>+{course.timeslots.length - 2}</Text>
                    )}
                  </View>
                )}
              </TouchableOpacity>
      ...
      {[{ visible: addModal, close: () => setAddModal(false), onSave: handleCreate, title: "Add course" },
        ...
        .map(({ visible, close, onSave, title: modalTitle }) => (
        <Modal key={modalTitle} visible={visible} animationType="slide" transparent onRequestClose={close}>
          <View style={[s.overlay, { backgroundColor: "rgba(0,0,0,0.4)" }]}>
            <View style={[s.sheet, { backgroundColor: colors.background }]}>
              <View style={s.sheetHeader}>
                <Text style={[s.sheetTitle, { color: colors.textPrimary }]}>{modalTitle}</Text>
                <TouchableOpacity onPress={close}><Ionicons name="close" size={22} color={colors.textSecondary} /></TouchableOpacity>
              </View>
              <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                {/* Name */}
                <Text style={[s.fieldLbl, { color: colors.textSecondary }]}>Course name *</Text>
                <TextInput style={[s.input, { backgroundColor: colors.card, borderColor: colors.cardBorder, color: colors.textPrimary }]}
                  placeholder="e.g. Fundamentals of Nursing" placeholderTextColor={colors.textSecondary}
                  value={form.name} onChangeText={v => setForm(f => ({ ...f, name: v }))} />

                {/* Code */}
                <Text style={[s.fieldLbl, { color: colors.textSecondary }]}>Subject code</Text>
                <TextInput style={[s.input, { backgroundColor: colors.card, borderColor: colors.cardBorder, color: colors.textPrimary }]}
                  placeholder="e.g. NCM 101" placeholderTextColor={colors.textSecondary}
                ...
                <View style={s.colorRow}>
                  {COURSE_COLORS.map(c => (
                    <TouchableOpacity key={c} onPress={() => setForm(f => ({ ...f, color: c }))}
                      style={[s.colorDot, { backgroundColor: c, borderWidth: form.color === c ? 3 : 0, borderColor: "#fff" }]}>
                  ...
                  <View key={idx} style={[s.tsRow, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                      <View style={{ flexDirection: "row", gap: 6 }}>
                        {DAYS.map(d => (
                          <TouchableOpacity key={d} onPress={() => updateTimeslot(idx, "day", d)}
                            style={[s.dayChip, { backgroundColor: ts.day === d ? form.color : colors.background, borderColor: ts.day === d ? form.color : colors.cardBorder }]}>
                            <Text style={[s.dayChipTxt, { color: ts.day === d ? "#fff" : colors.textSecondary }]}>{d.slice(0, 3)}</Text>
                          </TouchableOpacity>
                        ))}
                      </View>
                    </ScrollView>
                    <View style={s.tsTimeRow}>
                      <TextInput style={[s.tsTimeInput, { backgroundColor: colors.background, borderColor: colors.cardBorder, color: colors.textPrimary }]}
                ...
                <TouchableOpacity style={[s.submitBtn, { backgroundColor: colors.primary }, saving && { opacity: 0.6 }]}
                  ...
                  {saving ? <ActivityIndicator color="#fff" />
                    : <Text style={s.submitTxt}>{modalTitle === "Add course" ? "Save course" : "Save changes"}</Text>}
                </TouchableOpacity>
...
/>
                {/* Detail header */}
                <View style={[s.detailHeader, { borderBottomColor: colors.cardBorder, borderTopColor: selectedCourse.color, borderTopWidth: 4 }]}>
                  <View style={{ flex: 1 }}>
                    <Text style={[s.detailCode, { color: selectedCourse.color }]}>{selectedCourse.code || "Course"}</Text>
                    <Text style={[s.detailName, { color: colors.textPrimary }]} numberOfLines={2}>{selectedCourse.name}</Text>
                    {selectedCourse.instructor ? <Text style={[s.detailInstructor, { color: colors.textSecondary }]}>{selectedCourse.instructor}</Text> : null}
                  </View>
                ...
                <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1 }}>
                  ...
                  {selectedCourse.timeslots.length > 0 && (
                    <>
                      ...
                      {selectedCourse.timeslots.map((ts, i) => (
                        <View key={i} style={[s.tsDetailRow, { backgroundColor: selectedCourse.color + "15", borderColor: selectedCourse.color + "40" }]}>
                          <View style={[s.tsDayBadge, { backgroundColor: selectedCourse.color }]}>
                            <Text style={s.tsDayTxt}>{ts.day.slice(0, 3)}</Text>
                          </View>
                          <Text style={[s.tsDetailTime, { color: colors.textPrimary }]}>{ts.startTime} — {ts.endTime}</Text>
                        </View>
                      ))}
                    </>
                  )}
                  ...
                  {scheduleLoading ? (
                    ...
                    <View style={s.emptySchedule}>
                      ...
                      <Text style={[s.emptySub, { color: colors.textSecondary }]}>No schedules linked to this course yet.</Text>
                      <Text style={[s.emptySub, { color: colors.textSecondary, fontSize: 11 }]}>When adding a schedule, select this course.</Text>
                    </View>
                  ) : (
                    <>
                      {/* Pending */}
...
{done(courseSchedules).map(item => (
  ...
  />
                  ...
                  <View style={s.detailActions}>
                    <TouchableOpacity style={[s.detailActionBtn, { backgroundColor: colors.primaryLight }]} onPress={() => openEdit(selectedCourse)}>
                      <Ionicons name="pencil-outline" size={16} color={colors.primary} />
                      <Text style={[s.detailActionTxt, { color: colors.primary }]}>Edit</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[s.detailActionBtn, { backgroundColor: "#FCEBEB" }]} onPress={() => handleDelete(selectedCourse)}>
...
const crs = StyleSheet.create({
  ...
  borderRadius: 12,
  padding: 12,
  marginBottom: 10,       // ← increase from 8 to 10
  marginHorizontal: 16,   // ← must be present
},
  done:    { opacity: 0.5 },
...
const s = StyleSheet.create({
  ...
  // These already exist in s.detailSec — confirm it has paddingHorizontal: 16
detailSec: {
  fontSize: 11,
  fontWeight: "600",
  ...
  letterSpacing: 0.5,
  marginBottom: 10,
  marginTop: 20,        // ← was 14, increase to 20
